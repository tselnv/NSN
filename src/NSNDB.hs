...
-- prepareDB :: ConnectInfo -> IO Connection
-- prepareDB connInfo = do
--   conn <- connect connInfo
--   execute_ conn "CREATE TABLE IF NOT EXISTS podcasts (\
--                 \podcast_id INTEGER NOT NULL PRIMARY KEY, \
...
-- addToTable :: Connection -> Int -> String -> String -> IO Int64
-- addToTable dbh id url name  = execute dbh "INSERT INTO podcasts (podcast_id, podcast_url, podcast_name) VALUES (?,?,?)"
--                                            (id, url, name)


-- getFromTable :: Int -> Reader SessionEnv (IO [(Int, String, String)])
-- getFromTable id = do
--   dbh <- asks envConnection
--   return $ query dbh "SELECT podcast_id, podcast_url, podcast_name FROM podcasts WHERE podcast_id <> (?)" (Only id)


-- getStatus :: ReaderT SessionEnv IO String
-- getStatus = asks envStatus >>= liftIO . return


-- runDB :: Password -> IO ()
-- runDB pwd = do
--   conn <- prepareDB $ connInfo pwd
--   let snEnv = SessionEnv { envConnection = conn
--                          , envStatus = "Test status" }
--   result <- runReader (getFromTable 666) snEnv
--   print result
--   status <- runReaderT getStatus snEnv
--   print status
